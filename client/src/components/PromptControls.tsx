    ...
    <div className="rounded-lg border border-border bg-card/50 px-4 py-3 space-y-3 mb-3">
      ...
      <ControlSection label="Output Length">
        {LENGTHS.map((l) => (
          ...
          >
            {/* Length bars visual — bar heights grow with output length */}
            <span className="flex items-end gap-0.5">
              <span className={`w-0.5 h-2 rounded-full ${value.length === l.id ? "bg-primary-foreground" : "bg-muted-foreground/60"}`} />
              <span className={`w-0.5 rounded-full ${l.id === "medium" || l.id === "long" ? "h-3.5" : "h-1"} ${value.length === l.id ? "bg-primary-foreground" : "bg-muted-foreground/40"}`} />
              <span className={`w-0.5 rounded-full ${l.id === "long" ? "h-5" : "h-1.5"} ${value.length === l.id ? "bg-primary-foreground" : "bg-muted-foreground/20"}`} />
            </span>
            <span>{l.label}</span>
            <span className={`text-[9px] ${value.length === l.id ? "text-primary-foreground/70" : "text-muted-foreground/50"}`}>
              {l.desc}
            </span>
          </button>
        ))}
